...
VectorClock<>::TID VarState::is_rw_sh_race(
    ThreadState* t, xvector<std::size_t>* shared_vc) const {
...
std::vector<std::size_t>::iterator VarState::find_in_vec(
    VectorClock<>::TID tid, xvector<std::size_t>* shared_vc)
    const {  // TODO: make it public & add the shared_vc argument
...
VectorClock<>::VC_ID VarState::get_sh_id(
    uint32_t pos, xvector<std::size_t>* shared_vc) const {
